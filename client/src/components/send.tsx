...
export function Send() {
  ...
  const [modeAddr, setmodeAddr] = useState<string>(
  ...
  const [shadowIDError, setShadowIDError] = useState<boolean>(false);
  ...
  const debouncedAddr = useDebounce(modeAddr, 500);
  ...
  useEffect(() => {
    ...
    if (theirID.startsWith('https://shadow/#')) {
      setTheirID(theirID.replace('https://shadow/#', ''));
    } else {
      generateNewEphKey();
    }
  ...
  return (
    <div style={{ paddingTop: '1rem' }}>
      <p>
      ETH will be sent to a secret blockchain account that will hold the ETH temporarily.
      The user who owns the Shadow ID will have control over the secret account.
      </p>
      <form
        className="lane"
        onSubmit={() => {
      ...
      >
        <div className="input-container">
          <input
            ...
            spellCheck="false"
            autoComplete="off"
            placeholder="Enter receiver Shadow ID"
            onChange={handleIDInput}
          />
          <label htmlFor="shadowID">SHADOW ID</label>
        </div>
      </form>
      ...
      {isConnected && balance && (
        <>
          <div>
            ...
            >
              ...
              <button
                className="hbutton"
                color="success"
                disabled={!write || isLoading || amountError || shadowIDError}
                onClick={(e) => {
                  e.preventDefault();
                  write?.();
                }}
          ...
          {shadowIDError && (
            <div className="lane">
              <p className="message error">Invalid Shadow ID</p>
            </div>
          )}
