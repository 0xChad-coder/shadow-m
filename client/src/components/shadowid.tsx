...
export function ShadowID() {
  ...
  useEffect(() => {
    ...
    setMetaAddr('S' + base58.encode(addr));
  }, [spendingKey]);
  ...
  const handleCopy = useCallback(
    (e: React.MouseEvent<HTMLButtonElement>) => {
      e.stopPropagation();
      copyTextToClipboard(metaAddr ? `https://shadow/#${metaAddr}` : '').then(
        () => {
          setCopied(true);
          setTimeout(() => {
            setCopied(false);
  ...
  return (
    <div className="large-block shadow-pane ">
          ...
          <div className="shadow-pane-header">
            <div className="header-item" style={{ alignItems: 'center' }}>
              <div className="block-wide">
                ...
                <p className="label shadow-id-contracted">{metaAddr}</p>
              </div>
              <button
      ...
      <div
        className="shadow-pane-body"
        style={{ display: opened ? 'block' : 'none' }}
      >
        <p className="block">Share the Shadow ID to receive ETH</p>
        <div className="shadow-pane-header buttons">
          <div className="block-wide">
            <p className="label" style={{ textTransform: 'uppercase' }}>
              Shadow ID:
            </p>
            <p className="label shadow-id">{metaAddr}</p>
          </div>
        ...
        <div
          className="shadow-pane-header block message"
          style={{
            width: '90%',
