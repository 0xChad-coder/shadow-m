...
export function Withdraw() {
  ...
  const stealthAddressConfig = {
    address: stealthAddress[chain?.id || 919 || 34443],
    abi: STEALTH_ABI,
  };
  ...
  const withdraw = async (
    ...
    try {
      ...
      const gasPremium = ethers.utils.parseUnits("1.5", "gwei");
      const feeCap = ethers.utils.parseUnits("1.5", "gwei");
      const maxFeePerGas = ethers.utils.parseUnits('1.6', 'gwei');

      if(chain?.id == 919) {
        fee = (gasLimit.mul(maxFeePerGas));
        console.log(`mode: ${fee.toNumber()}`);
      } else {
        fee = (gasLimit.mul(gasPrice)).add(1000000000);
      ...
      if(chain?.id == 919) {
        config.request = {
          ...
          maxFeePerGas: feeCap, // New parameter for EIP-1559
          maxPriorityFeePerGas: gasPremium, // New parameter for EIP-1559
          gasLimit: gasLimit
        };
      } else {
        config.request = {
